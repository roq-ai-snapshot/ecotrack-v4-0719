...
import { getUserResourceById, updateUserResourceById } from 'apiSdk/user-resources';
...
function UserResourceEditPage() {
  ...
  const handleSubmit = async (values: UserResourceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateUserResourceById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
